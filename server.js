...
function send(res, status, body) { res.writeHead(status, jsonHeaders); res.end(JSON.stringify(body)) }
async function readBody(req) { let raw = ''; for await (const chunk of req) raw += chunk; if (raw.length > 1_000_000) throw new Error('Request is too large.'); return raw ? JSON.parse(raw) : {} }

async function localGenerate(localUrl, model, prompt) {
  const r = await fetch(`${localUrl.replace(/\/$/, '')}/api/generate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model, prompt, stream: false, options: { temperature: 0.1 } }) })
...
async function compressWithLocal({ localUrl, localModel, text, direction, targetRatio = 0.55 }) {
  const task = direction === 'request' ? 'Rewrite the request compactly while preserving every requirement, constraint, number, named entity, and desired output format.' : 'Rewrite the answer compactly while preserving facts, caveats, steps, code, and formatting needed by the user.'
  return localGenerate(localUrl, localModel, `You are a lossless token-compression layer. ${task}\n\nRules:\n- Return only the rewritten content.\n- Do not invent, remove, or change facts.\n- Keep code valid and identifiers exact.\n- Aim for about ${Math.round(targetRatio * 100)}% of the original tokens; preserve meaning over the target.\n\nContent:\n${text}`)
}
...
async function callProvider(provider, apiKey, model, messages) {
  ...
  if (provider === 'anthropic') {
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }, body: JSON.stringify({ model: model || 'claude-3-5-haiku-latest', max_tokens: 4096, ...(system ? { system } : {}), messages: messages.filter(m => m.role !== 'system') }) })
...
async function requestWithAvailableProvider({ provider, apiKeys, models, messages }) {
